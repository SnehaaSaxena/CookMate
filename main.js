...
function displayRecipes(recipes){
   ...
   recipes.forEach((recipe)=>{
       ...
       recipeDiv.dataset.id=recipe.idMeal;// .dataset gives access to all data attributes of the element
...
async function getRandomRecipe(){
   ...
   try{
     ...
     showMessage("Failed to fetch a random recipe. Please check your internet connection and try again",true);
   }
};

function showModal(){
   modal.classList.remove("hidden");
   document.body.style.overflow="hidden"; // to remove background scrolling
}

function closeModal(){
   modal.classList.add("hidden");
   document.body.style.overflow=""; // to add scrolling back
}
...
async function getRecipeDetails(id){
   ...
   try{
     ...
     if(data.meals && data.meals.length>0){
       ...
       modalContent.innerHTML=`<p class="message error">Could not load recipe detsils.</p>`;
     }
   }catch(error){
...
modal.addEventListener("click",(e)=>{
   if(e.target === modal){ // if user clicks on bg
      closeModal();
   }
});
...
function displayRecipeDetails(recipe){
   ...
   const ingredientHTML= ingredients.length ? `<h3>Ingredients</h3><ul>${ingredients.join("")}</ul>` : "";
   ...
   const youtubeHTML= recipe.strYoutube ? `<h3>Video Recipe</h3> <div class="video-wrapper"> <a href="${recipe.strYoutube
   }" target="_blank">Watch on YouTube</a> </div>` : "";
   const sourceHTML = recipe.strSource ? `<div class="source-wrapper">
   <a href="${recipe.strSource}" target="_blank">View Original Source</a></div>` : "";
